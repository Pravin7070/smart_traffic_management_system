...
const SUMOSignals = (() => {
    ...
    function findBusiestDirection(detectorData, excludeDir) {
        ...
        for (const dir of dirs) {
            ...
            if (detectorData && detectorData[dir]) {
                ...
                score = (d.queueLength || 0) * 2 + (d.vehicleCount || 0);
            }
    ...
    function updateGreenPhase(dt, detectorData) {
        ...
        // 2. After minimum green: check if current direction still has vehicles
            ...
            const hasVehicles = detectorData && detectorData[currentGreenDir] &&
                detectorData[currentGreenDir].vehiclesOnDetector > 0;
            ...
            const currentQueue = (detectorData && detectorData[currentGreenDir]) ?
                (detectorData[currentGreenDir].queueLength || 0) : 0;

            // If another direction has 3x more vehicles, switch early
            if (busiest.score > 0 && busiest.score > currentQueue * 3 && phaseTimer >= config.minGreen) {
                shouldSwitch = true;
            }
        }
