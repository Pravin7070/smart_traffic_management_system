...
const SUMOVehicles = (() => {
    const vTypes = {
        car: { id: 'car', label: 'CAR', length: 28, width: 16, accel: 2.6, decel: 4.5, maxSpeed: 13.89, minGap: 24, sigma: 0.3, color: '#3b82f6', yoloColor: '#00ff88', probability: 0.5 },
        bus: { id: 'bus', label: 'BUS', length: 55, width: 20, accel: 1.2, decel: 4.0, maxSpeed: 11.11, minGap: 34, sigma: 0.2, color: '#f59e0b', yoloColor: '#ffbb00', probability: 0.12 },
        truck: { id: 'truck', label: 'TRUCK', length: 48, width: 20, accel: 1.0, decel: 3.5, maxSpeed: 10.0, minGap: 34, sigma: 0.2, color: '#ef4444', yoloColor: '#ff3355', probability: 0.13 },
        bike: { id: 'bike', label: 'BIKE', length: 16, width: 10, accel: 3.5, decel: 5.0, maxSpeed: 8.33, minGap: 16, sigma: 0.3, color: '#a855f7', yoloColor: '#bf00ff', probability: 0.15 },
        emergency: { id: 'emergency', label: 'AMBULANCE', length: 36, width: 18, accel: 3.0, decel: 5.0, maxSpeed: 16.67, minGap: 26, sigma: 0.1, color: '#ff0044', yoloColor: '#ff0044', probability: 0 }
    };
    ...
    const MAX_IN_JUNCTION_TOTAL = 3;

    function axisOfDir(dir) {
        return (dir === 'north' || dir === 'south') ? 'ns' : 'ew';
    }
    ...
    function spawnVehicle(dir, vType = null, laneIndex = -1) {
        ...
        const vehicle = {
            ...
            accel: type.accel * SPEED_SCALE * 0.45,
            decel: type.decel * SPEED_SCALE * 0.4,
            ...
            displayName: dir === 'south' ? SOUTH_LANE_DISPLAY_NAME : '',
    ...
    function countInJunctionTotal() {
        let count = 0;
        for (const v of vehicles) {
            if (v.active && v.inJunction && !v.exiting) count++;
        }
        return count;
    }

    function getOccupiedJunctionAxis() {
        for (const v of vehicles) {
            if (v.active && v.inJunction && !v.exiting) return axisOfDir(v.dir);
        }
        return null;
    }
    ...
    function update(dt, signalStates) {
        ...
        for (const v of vehicles) {
            ...
            // If a vehicle is stuck too long on approach while green and with space ahead,
            // force a gentle restart to avoid pre-stop-line deadlocks.
            if (!v.inJunction && !v.exiting && signalState === 'green' && v.stuckTime > APPROACH_STUCK_TIMEOUT) {
                const leaderGap = leader ? getGap(v, leader) : 999;
                if (distToStop > 8 && leaderGap > Math.max(18, v.minGap * 0.6)) {
                    v.stopped = false;
                    targetSpeed = Math.max(targetSpeed, Math.min(v.maxSpeed * 0.35, 2.2));
                    v.stuckTime = 0;
                }
            }

            // ===== JUNCTION TRAVERSAL & TURNING =====
            // Run this even for temporarily stopped vehicles so they can recover from deadlocks.
            if (!v.exiting) {
                // Enter junction
                if (!v.inJunction && !v.stopped && distToStop <= 0) {
                    // Double-check junction limit at entry point
                    const inJunctionCount = countInJunction(v.dir);
                    const totalInJunction = countInJunctionTotal();
                    const occupiedAxis = getOccupiedJunctionAxis();
                    const enteringAxis = axisOfDir(v.dir);
                    const crossAxisBlocked = occupiedAxis && occupiedAxis !== enteringAxis;

                    if (inJunctionCount >= MAX_IN_JUNCTION_PER_DIR || totalInJunction >= MAX_IN_JUNCTION_TOTAL || crossAxisBlocked) {
                ...
                if (v.inJunction) {
                    v.junctionTime += dt;
                    const forceClear = v.junctionTime > JUNCTION_STUCK_TIMEOUT;
                    const hardForceClear = v.junctionTime > JUNCTION_STUCK_TIMEOUT * 2;
                    ...
                    const conflict = forceClear ? null : findJunctionConflict(v);
                    if (conflict) {
                    ...
                    if (forceClear) {
                        v.stopped = false;
                        targetSpeed = Math.max(targetSpeed, v.maxSpeed * 0.55);
                    }

                    // Hard fail-safe: if still trapped in center for too long, force vehicle to exit.
                    if (hardForceClear) {
                        if (v.turnIntent !== 'straight' && v.exitTarget) {
                            v.x = v.exitTarget.x;
                            v.y = v.exitTarget.y;
                            ...
                            const pushDist = junctionSize + 50;
                            if (v.dir === 'north') v.y = cy + pushDist;
                            else if (v.dir === 'south') v.y = cy - pushDist;
                            else if (v.dir === 'east') v.x = cx - pushDist;
                            else if (v.dir === 'west') v.x = cx + pushDist;
                        }
                        v.inJunction = false;
                        v.exiting = true;
                        v.turning = false;
                        v.stopped = false;
                        v.junctionTime = 0;
                        targetSpeed = Math.max(targetSpeed, v.maxSpeed * 0.6);
                    }
                ...
                if (!v.inJunction) v.junctionTime = 0;
            }
            ...
            if (!v.stopped) {
                ...
                if (targetSpeed === 0) {
                    ...
                    sf = 0.35; // Faster acceleration for smoother recovery
                }
