...
const Renderer = (() => {
    ...
    function render(dt) {
        ...
        // Draw mode indicator (AUTO/MANUAL)
        drawModeIndicator(w, h);

        // Draw multi-intersection sync visualization
        drawMultiIntersectionPanel(w, h);
    ...
    function drawModeIndicator(w, h) {
        const isManual = ManualControl && ManualControl.isActive;
        const mode = isManual ? 'MANUAL CONTROL' : 'AUTOMATIC MODE';
        const color = isManual ? '#ffbb00' : '#00ff88';
        const bgColor = isManual ? 'rgba(255, 187, 0, 0.15)' : 'rgba(0, 255, 136, 0.08)';
        
        ctx.save();
        ctx.font = 'bold 14px "Courier New", monospace';
        
        const padding = 12;
        const textMetrics = ctx.measureText(mode);
        ...
        // Glow effect for manual mode
        if (isManual) {
            ctx.shadowColor = 'rgba(255, 187, 0, 0.5)';
            ctx.shadowBlur = 10;
            ctx.shadowOffsetX = 0;
            ctx.shadowOffsetY = 0;
        }
        ...
        ctx.fillText(mode, boxX + padding, boxY + boxH / 2);
        
        ctx.restore();
    }

    function drawMultiIntersectionPanel(w, h) {
        const stats = MultiIntersection.getStats();
        const panelW = 280;
        const panelH = 120;
        ...
        ctx.textAlign = 'left';
        ctx.fillText('MULTI-INTERSECTION SYNC', panelX + 10, panelY + 20);

        // Intersection 1 (Main)
        const mainInfo = MultiIntersection.getIntersectionInfo('main');
        ctx.font = '9px JetBrains Mono';
        ctx.fillStyle = 'rgba(191, 0, 255, 0.8)';
        ctx.fillText('🚦 MAIN', panelX + 10, panelY + 40);
        ctx.fillStyle = 'rgba(0, 240, 255, 0.6)';
        ctx.fillText(`Status: ACTIVE`, panelX + 60, panelY + 40);

        // Intersection 2 (Secondary)
        const secondaryInfo = MultiIntersection.getIntersectionInfo('secondary');
        ctx.fillStyle = 'rgba(0, 255, 136, 0.8)';
        ctx.fillText('🚦 SECONDARY', panelX + 10, panelY + 55);
        const syncState = secondaryInfo?.syncState || 'GREEN';
        const syncColor = syncState === 'GREEN' ? 'rgba(0, 255, 136, 0.8)' : 'rgba(255, 187, 0, 0.8)';
        ctx.fillStyle = syncColor;
        ctx.fillText(`Status: ${syncState}`, panelX + 60, panelY + 55);

        // Sync mode
        ctx.fillStyle = 'rgba(0, 240, 255, 0.6)';
        ctx.fillText(`Sync Mode:`, panelX + 10, panelY + 75);
        ctx.fillStyle = 'rgba(0, 240, 255, 0.8)';
        ctx.fillText(stats.sync_mode.toUpperCase(), panelX + 90, panelY + 75);

        // Offset
        ctx.fillStyle = 'rgba(0, 240, 255, 0.6)';
        ctx.fillText(`Offset: ${stats.sync_offset.toFixed(1)}s`, panelX + 10, panelY + 90);
        ...
        ctx.setLineDash([5, 5]);
        ctx.beginPath();
        ctx.moveTo(panelX + 50, panelY + 45);
        ctx.lineTo(panelX + 50, panelY + 53);
        ctx.stroke();
        ctx.setLineDash([]);
    }
    ...
    function drawHelpOverlay(w, h) {
        ...
        ctx.fillText('KEYBOARD SHORTCUTS', w / 2, 50);
        ...
        const shortcuts = [
            ['SPACE', 'Pause / Resume'],
            ['E', 'Emergency Vehicle'],
            ['S', 'Toggle Speed (1x, 2x, 4x, 0.5x)'],
            ['', ''],
            ['G', 'Green-Wave Sync Mode'],
            ['A', 'Alternating Sync Mode'],
            ['O', 'Offset Sync Mode'],
            ['I', 'Independent (No Sync)'],
            ['. (period)', 'Increase Offset'],
            [', (comma)', 'Decrease Offset'],
            ['', ''],
        ...
        shortcuts.forEach(([key, desc], idx) => {
            const x = idx < 6 ? leftX : rightX;
            const currentY = 100 + (idx % 6) * 35;

            if (key === '') {
                // Spacer
                ctx.fillStyle = 'rgba(0, 240, 255, 0.1)';
                ctx.fillRect(x - 10, currentY - 15, 250, 1);
                return;
            }

            // Key box
            ctx.fillStyle = 'rgba(0, 240, 255, 0.2)';
            ctx.fillRect(x, currentY - 18, 100, 26);
            ctx.strokeStyle = 'rgba(0, 240, 255, 0.5)';
            ctx.lineWidth = 1;
            ctx.strokeRect(x, currentY - 18, 100, 26);

            // Key text
            ctx.fillStyle = 'rgba(0, 240, 255, 0.95)';
            ctx.font = 'bold 12px JetBrains Mono';
            ctx.fillText(key, x + 8, currentY + 2);

            // Description
            ctx.fillStyle = 'rgba(200, 200, 200, 0.8)';
            ctx.font = '11px JetBrains Mono';
            ctx.fillText(desc, x + 110, currentY + 2);
        });
        ...
        ctx.fillStyle = 'rgba(0, 240, 255, 0.6)';
        ctx.textAlign = 'center';
        ctx.fillText('Press H to close this help | Multi-Intersection Sync v1.0', w / 2, h - 30);
    }
    ...
    function drawYOLOBox(v) {
        ...
        const label = v.displayName
            ? `${v.type.label} ${v.displayName}`
            : `${v.type.label} ${v.confidence}`;
        ctx.font = '9px JetBrains Mono';
    ...
    function drawAlertOverlays() {
        ...
        // Define lane regions for overlays
        const laneRegions = {
            'north': { x: cx - roadW / 2, y: cy - roadH, w: roadW, h: 40, color: 'rgba(255, 0, 0, 0.2)' },
            'south': { x: cx - roadW / 2, y: cy + roadH - 30, w: roadW, h: 40, color: 'rgba(255, 0, 0, 0.2)' },
            'east': { x: cx + roadW - 30, y: cy - roadH / 2, w: 40, h: roadH, color: 'rgba(255, 0, 0, 0.2)' },
            'west': { x: cx - roadW + 30, y: cy - roadH / 2, w: 40, h: roadH, color: 'rgba(255, 0, 0, 0.2)' }
        };
        
        alerts.forEach(alert => {
            const alertLocation = alert.location && alert.location.toLowerCase();
            const region = laneRegions[alertLocation];
            
            if (region && alert.type === 'ACCIDENT') {
                ctx.save();
                
                // Draw pulsing red overlay for accidents
                const pulse = Math.sin(performance.now() / 200) * 0.3 + 0.4;
                ctx.fillStyle = `rgba(255, 51, 85, ${pulse * 0.4})`;
                ...
                ctx.textAlign = 'center';
                ctx.textBaseline = 'middle';
                ctx.fillText('⚠️', region.x + region.w / 2, region.y + region.h / 2);
                
                ctx.restore();
            } else if (region && alert.type === 'HEAVY_CONGESTION') {
                ctx.save();
                
                // Draw pulsing yellow overlay for congestion
                const pulse = Math.sin(performance.now() / 300) * 0.2 + 0.3;
                ctx.fillStyle = `rgba(255, 187, 0, ${pulse * 0.3})`;
                ctx.fillRect(region.x, region.y, region.w, region.h);
